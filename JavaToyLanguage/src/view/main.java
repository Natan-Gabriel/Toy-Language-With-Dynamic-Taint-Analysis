...
//		}).collect(Collectors.toMap(data ->(Integer) data[0], data -> (IStmt) data[1]));
		...
		Map<Integer, IStmt> map3 = Stream.of(new Object[][] {
				{1,new VarDeclStmt("x",new IntType(),1)},{2,new VarDeclStmt("y",new IntType(),2)},{3,new VarDeclStmt("a",new IntType(),3)},{4,new AssignStmt("x",new ValueExp(new IntValue(2)),4)},{5,new AssignStmt("y",new ValueExp(new IntValue(3)),5)},
