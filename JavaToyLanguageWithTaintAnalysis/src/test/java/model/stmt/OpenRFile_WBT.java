...
public class OpenRFile_WBT {
    ...
    public void tc_FileNameIsNotAString() throws CustomException, VarNotDefined, DivByZero, TaintedAddress, VarIsDefined {
        ...
        boolean aux=true;
        try {
            IStmt stmt1 = new OpenRFile(new ValueExp(new BoolValue(true)),1);
            stmt1.execute(prg);
        }
    ...
    public void tc_FileIsOpened() throws CustomException, VarNotDefined, DivByZero, TaintedAddress, VarIsDefined {
        ...
        boolean aux=true;
        try {
            IStmt stmt1 = new OpenRFile(new ValueExp(new StringValue("files/test_open.txt")),1);
            stmt1.execute(prg);
            stmt1.execute(prg);
    ...
    public void tc_FileDoesNotExist() throws CustomException, VarNotDefined, DivByZero, TaintedAddress, VarIsDefined {
        ...
        boolean aux=true;
        try {
            IStmt stmt1 = new OpenRFile(new ValueExp(new StringValue("files/inexistent_file.txt")),1);stmt1.execute(prg);
            stmt1.execute(prg);
        }
        catch(Exception e){
            aux=false;
